...
if (not isDedicated) spawn {
	...
	#define STR_HELP "Скрипт на большие потери сторон (Автор: Zealot) <br/>Скрипт завершает игру автоматически при потере одной из сторон боя более чем 90 % личного состава."
	#define STR_SCRIPTS_NAME "Скрипты"
	#define STR_SCRIPT_NAME "Скрипт на большие потери"
...
if (not isServer) exitWith {diag_log "LOSSES.SQF NOT SERVER";};
...
//		diag_log str ["LOSSES.sQF ", _enemies, _beginEnemies, _aliveEnemies ] ;
		...
		if (_beginEnemies > 0) then {
			if (_aliveEnemies / _beginEnemies < 0.1 ) then {
