...
while {zlt_pub_frz_state < 3} do {
	...
	_infoStr1 = format ["<t size='0.75' color='%2'>До старта: %1 сек.</t>", 0 max (_sec min _time3), if (_sec >= 30 ) then {"#00ff00"} else {"#ff0000"}];
	...
	if (count zlt_pub_frz_waitUsrs != 0 or count zlt_pub_frz_startUsrs != 0) then {
		_infoStr5 = format ["<t size='0.5' color='#ff0000'>За старт игры: %1<br/>За продолжение подготовки: %2<br/></t>",
		[zlt_pub_frz_startUsrs] call zlt_fnc_list_from_array, [zlt_pub_frz_waitUsrs] call zlt_fnc_list_from_array ];
		[_infoStr5, 0, safeZoneY + 0.1,1.5,0,0,4] spawn bis_fnc_dynamicText;
